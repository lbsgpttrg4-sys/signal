...
# === Boom helper RSI (EMA-based, internal use only) ===
def compute_rsi_ema(series: pd.Series, period: int) -> pd.Series:
...
def rolling_slope(series: pd.Series, win: int) -> pd.Series:
    vals = series.values.astype(float)
    n = len(vals)
    slopes = np.full(n, np.nan, dtype=float)
    x_full = np.arange(n).astype(float)
    for i in range(win - 1, n):
        y = vals[i - win + 1: i + 1]
        if np.isnan(y).sum() > win // 3:
            continue
        x = x_full[i - win + 1: i + 1]
        x = (x - x.mean()) / (x.std() + 1e-12)
        yc = y - np.nanmean(y)
        denom = np.nansum(x * x) + 1e-12
        slopes[i] = np.nansum(x * yc) / denom
    return pd.Series(slopes, index=series.index)

def predict_boom_sma_rsi1(price: pd.DataFrame, cfg: dict) -> pd.Series:
    smaN = cfg["sma_rsi_reclaim_sma"]
    sma = price["Price"].rolling(smaN, min_periods=smaN//2).mean()
    rsi = compute_rsi_ema(price["Price"], period=cfg["rsi_period"])  # <-- use internal RSI
    slope_src = price["Price"].rolling(30, min_periods=15).mean()
    slope = rolling_slope(slope_src, win=cfg["sma_rsi_min_slope_win"])

    q_feat = pd.DataFrame({
        "above_sma_pct": (price["Price"] > sma).astype(int).resample("QE").mean(),
        "rsi_mean": rsi.resample("QE").mean(),
        "slope_mean": slope.resample("QE").mean()
    })

    boom = (
        (q_feat["above_sma_pct"] >= cfg["sma_rsi_above_sma_pct"]) &
        (q_feat["rsi_mean"] < cfg["sma_rsi_rsi_limit"]) &
        (q_feat["slope_mean"] > cfg["sma_rsi_min_slope"])
    )
    return boom.fillna(False)
